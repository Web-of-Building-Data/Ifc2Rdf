...
import fi.hut.cs.drumbeat.ifc.data.model.IfcLiteralValueList;
...
public abstract class IfcStepEntity {
	...
	protected <T> List<T> getListValue(String attributeName) {
		IfcLiteralValueList value = (IfcLiteralValueList)values.get(attributeName);

		if (value == null) {
			value = (IfcLiteralValueList)entity.getLiteralAttributes().selectFirstByName(attributeName)
					.getValue();
